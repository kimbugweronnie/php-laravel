...
use App\Models\WorkloadAnalysis;
...
class HtsService extends Controller
{
    private $reportgroup;
    private $report;
    private $reports;
    ...
    public function htsreport($querygroupid,$fromdate,$todate)
    {
        $error = [];
        $fn = [];
        $pr = [];
        ...
        foreach ($querydata as $query) {
            ...
            }elseif(strpos($query[2],'TotalANC1') !== false ){
                array_push($error,$query[0],$query[2]);
            }elseif(strpos($query[2],'a.pregnantatartstart') !== false ){
                array_push($error,$query[0],$query[2]);
        ...
        foreach ($fn as $record) {
            ...
            array_push($data,['name'=>$record[0],"querygroupid"=>$querygroupid,"contentdata"=>$querydata]);
        }
