    ...
    <div class="row">
        <nav>
            <div class="nav nav-tabs" id="nav-tab" role="tablist">
                ...
                <a wire:click="handleRequest" class="nav-link text-capitalize {{ $reqClicked ? 'active' : '' }}" id="nav-request-tab" data-bs-toggle="tab" data-bs-target="#nav-request" href="#requests" type="button" role="tab" aria-controls="nav-request" aria-selected="false">Requests</a>
            </div>
        </nav>
        <div class="tab-content" id="nav-tabContent">
                                    ...
                                    <table class="table table-hover">
                                        <thead>
                                            <tr>
                                                ...
                                                <th class="text-capitalize" scope="col">Template name</th>
                                        ...
                                        <tbody>
                                            @foreach($memoTypes as $memoType)
                                                <tr>
                                                    ...
                                                    <td class="text-capitalize">{{ Str::lower($memoType['document_name']) }}</td>
                                                    <td>{{ $memoType['order_number'] }}</td>
                                                    <td>
                                                        <a wire:click="createMemoType({{ $memoType['id'] }})" class="btn btn-sm btn-outline-primary text-capitalize rounded">
                                                            Create request
                                                        </a>
                                                    </td>
                                                </tr>
            ...
            <div class="tab-pane fade {{ $reqClicked ? 'show active' : '' }}" id="nav-request" role="tabpanel" aria-labelledby="nav-request-tab" tabindex="0">
                <div class="row">
                    <div class="col-lg-12 mb-5">
                        <div class="d-flex justify-content-between my-3">
                            <h4 class="text-capitalize fw-bold">List of {{ $authStatus ? 'authorized' : '' }} {{ $unAuthStatus ? 'Unauthorized' : ''}} {{ $rejectStatus ? 'Rejected' : ''}} memo requests</h4>
                            <div>
                                ...
                                <table class="table table-hover">
                                    <thead>
                                        <tr>
                                            ...
                                            <th class="text-capitalize" scope="col">Request name</th>
                                            <th scope="col">Project</th>
                                            <th scope="col">Station </th>
                                            <th class="text-capitalize" scope="col">Order number</th>
                                    ...
                                    <tbody>
                                        @foreach($memoReqs as $memoReq)
                                            <tr>
                                                <td class="text-capitalize">{{ Carbon\Carbon::parse($memoReq['date_added'])->format('d/m/Y') }}</td>
                                                <td><a class="text-decoration-none" href="{{ route('memos.show', $memoReq['id']) }}">{{ $memoReq['fields']['title'] }}</a></td>
                                                <td>{{ $memoReq['related_project_document']['related_project']['project_name'] }}</td>
                                                <td class="text-capitalize">{{ Str::lower($memoReq['related_project_document']['related_region_project']['station_name']) }}</td>
                                                <td>{{ $memoReq['related_project_document']['order_number'] }}</td>
                                                <td>{{ $memoReq['added_by']['first_name'] }} {{ $memoReq['added_by']['last_name'] }}</td>
                                            </tr>
                                        @endforeach
                                    </tbody>
                                </table>
