                                        ...
                                        <tbody>
                                            @foreach($procurementTypes as $procurementType)
                                                <tr>
                                                    ...
                                                    <td class="text-capitalize">{{ Str::lower($procurementType['document_name']) }}</td>
                                                    <td>{{ $procurementType['order_number'] }}</td>
                                                    <td>
                                                        <a wire:click="createProcurementType({{ $procurementType['id'] }})" class="btn btn-sm btn-outline-primary text-capitalize rounded" href="#">
                                                            Create request
                                                        </a>
                                                    </td>
                                                </tr>
                ...
                <div class="row">
                    <div class="col-lg-12 mb-5">
                       <div class="d-flex justify-content-between my-3">
                            <h4 class="text-capitalize fw-bold">List of {{ $authStatus ? 'authorized' : '' }} {{ $unAuthStatus ? 'Unauthorized' : ''}} {{ $rejectStatus ? 'Rejected' : ''}} procurement requests</h4>
                            <div>
                       ...
                       <div class="border bg-white rounded-3 shadow px-4 py-3 mt-4">
                            @if (count($procurementReqs) > 0)
                                <table class="table table-hover">
                                    ...
                                    <tbody>
                                        @foreach($procurementReqs as $procurementReq)
                                            <tr>
                                                <td class="text-capitalize">{{ Carbon\Carbon::parse($procurementReq['date_added'])->format('d/m/Y') }}</td>
                                                <td class="text-capitalize"><a class="text-decoration-none" wire:navigate href="{{ route('procurements.show', $procurementReq['id']) }}">{{ Str::lower($procurementReq['fields']['title']) }}</a></td>
                                                <td class="text-capitalize">{{ Str::lower($procurementReq['related_project_document']['document_name']) }}</td>
                                                <td>{{ $procurementReq['related_project_document']['related_project']['project_name'] }}</td>
                                                <td class="text-capitalize">{{ Str::lower($procurementReq['related_project_document']['related_region_project']['station_name']) }}</td>
                                                <td>{{ $procurementReq['related_project_document']['order_number'] }}</td>
                                            </tr>
                                        @endforeach
                                    </tbody>
                                </table>
