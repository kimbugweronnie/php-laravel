<div class="container-fluid p-0">
    <div class="row">
        <div class="col-lg-12 d-flex justify-content-between">
            <div>
                <a class="btn btn-secondary px-3" href="{{ route('activities.index') }}">
                    <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" class="bi bi-box-arrow-in-left mb-1" viewBox="0 0 16 16">
                        <path fill-rule="evenodd" d="M10 3.5a.5.5 0 0 0-.5-.5h-8a.5.5 0 0 0-.5.5v9a.5.5 0 0 0 .5.5h8a.5.5 0 0 0 .5-.5v-2a.5.5 0 0 1 1 0v2A1.5 1.5 0 0 1 9.5 14h-8A1.5 1.5 0 0 1 0 12.5v-9A1.5 1.5 0 0 1 1.5 2h8A1.5 1.5 0 0 1 11 3.5v2a.5.5 0 0 1-1 0v-2z"/>
                        <path fill-rule="evenodd" d="M4.146 8.354a.5.5 0 0 1 0-.708l3-3a.5.5 0 1 1 .708.708L5.707 7.5H14.5a.5.5 0 0 1 0 1H5.707l2.147 2.146a.5.5 0 0 1-.708.708l-3-3z"/>
                    </svg>
                    Back
                </a>
            </div>
             @if(Session::get('employee_details')['has_department_role'] == false  || Session::has('projects'))
                <div>
                    <button type="button" class="btn btn-outline-primary px-3" wire:click="createTask" href="{{ route('tasks.create') }}">
                        <svg xmlns="http://www.w3.org/2000/svg" width="26" height="26" fill="currentColor" class="bi bi-plus" viewBox="0 0 18 18">
                            <path d="M8 4a.5.5 0 0 1 .5.5v3h3a.5.5 0 0 1 0 1h-3v3a.5.5 0 0 1-1 0v-3h-3a.5.5 0 0 1 0-1h3v-3A.5.5 0 0 1 8 4"/>
                        </svg>
                        Add task
                    </button>
                </div>
            @endif
        </div>
    </div>

    <div class="row mt-3">
        <div class="col-lg-12">
            <div class="border rounded-3 bg-white shadow py-4 px-5">
                <div class="mb-2 fw-bold text-capitalize fs-4">
                    Activity name: {{ $activity['activity_name'] }}
                </div>
                @forelse($activity['related_tasks'] as $task)
                    <div class="mb-2">
                        <span class="fw-bold text-capitalize">Task name:</span> <a class="text-decoration-none" href="{{ route('tasks.show', $task['id']) }}">{{ $task['task_name'] }}</a>
                    </div>
                    <div class="mb-2">
                        <span class="fw-bold text-capitalize">Start date:</span> {{ $task['start_date'] }}
                    </div>
                    <div class="mb-2">
                        <span class="fw-bold text-capitalize">End date:</span> {{ $task['end_date'] }}
                    </div>
                    
                    @if (!$loop->last)
                        <hr />
                    @endif
                @empty
                    <div>No tasks available</div>
                @endforelse
            </div>
        </div>
    </div>
</div>
