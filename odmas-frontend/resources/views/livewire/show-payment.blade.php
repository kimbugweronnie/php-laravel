...
<div class="container-fluid p-0">
    <div class="row">
        ...
        <div class="col-lg-12 d-flex justify-content-between">
            <div>
                <a class="btn btn-outline-secondary px-3" href="{{ route('payments.index') }}">
            ...
            <div class="d-flex justify-content-between">
                @if (is_null($this->isItApproved()))
                    <a class="btn btn-outline-secondary me-3"
                        href="{{ route('payments.editPayment', $paymentReq['related_document_request']['id']) }}">
                        <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor"
                ...
                <a class="btn btn-outline-secondary" id="openPdfLink"
                    href="{{ route('payment.pdf', $paymentReq['related_document_request']['id']) }}">
                    <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor"
    ...
    <div class="row mt-3">
        <div class="col-lg-12">
            <div class="border bg-white rounded-3 shadow p-4 custom-tab">
                <div class="fs-4 text-uppercase fw-bold text-center pb-4">
                    <div><img src="{{ asset('img/full-logo.png') }}" width="140" alt=" payment image"></div>
                    <div>Uganda episcopal conference</div>
                    <div>Uganda catholic secretariat</div>
                    <div>Uganda Catholic medical bureau</div>
                    <div>
                        {{ $paymentReq['related_document_request']['related_project_document']['related_project']['project_name'] }}
                    </div>
                ...
                <div>
                    <div class="row mb-3">
                        <div class="col-6 fs-4"><span class="fw-bold">Requisitioning Officer:</span>
                            {{ $paymentReq['related_document_request']['added_by']['first_name'] }}
                            {{ $paymentReq['related_document_request']['added_by']['last_name'] }}</span> </div>
                        <div class="col-6 fs-4"><span class="fw-bold">Source of Funds:</span></div>
                    </div>
                    <div class="fs-4 mb-3"><span class="fw-bold">Purpose:</span> {{ $paymentReq['title'] }}</div>
                </div>
                <table class="table table-bordered">
                    <tbody>
                        <tr>
                            <td class="fw-bold text-capitalize">Date</td>
                            <td class="fw-bold text-capitalize" colspan="2">Description</td>
                            <td class="fw-bold text-capitalize" colspan="2">Budget code</td>
                            <td class="fw-bold text-capitalize" colspan="2">
                                Amount({{ $paymentReq['currency_id']['currency_symbol'] }})</td>
                        </tr>
                        ...
                        @empty
                            <tr>
                                ...
                                <td colspan="2"></td>
                                <td colspan="2"></td>
                                <td colspan="2">{{ $this->getTotal($paymentReq['related_document_request']['id']) }}
                                </td>
                            </tr>
                        @endforelse
                        <tr>
                            ...
                            <td class="fw-bold" colspan="2">
                                {{ number_format($this->getTotal($paymentReq['related_document_request']['id']), 2) }}
                            </td>
                        </tr>
                        <tr>
                            <td class="fw-bold text-uppercase" colspan="3">Budget financing source:</td>
                            <td colspan="4"></td>
                        </tr>
                        <tr>
                            <td class="fw-bold text-uppercase">Cost center</td>
                            <td class="fw-bold text-uppercase">Project</td>
                            <td class="fw-bold text-uppercase">Donor</td>
                            <td class="fw-bold text-uppercase">Project category</td>
                            <td class="fw-bold text-uppercase">Budget Category</td>
                            <td class="fw-bold text-uppercase">Activity</td>
                            <td class="fw-bold text-uppercase">Sub activity</td>
                        </tr>
                        <tr>
                            <td class="text-uppercase">{{ $paymentReq['cost_center']['cost_center_name'] }}</td>
                            <td class="text-uppercase">
                                {{ $paymentReq['related_document_request']['related_project_document']['related_project']['project_name'] }}
                            </td>
                            <td class="text-uppercase"></td>
                            <td></td>
                            <td class="text-uppercase">{{ $paymentReq['budget_category'] }}</td>
                            <td class="text-uppercase">
                                {{ $paymentReq['related_activity'] ? $paymentReq['related_activity']['activity_name'] : null }}
                            </td>
                            <td></td>
                        </tr>
                    </tbody>
                        ...
                        <tbody>
                            @foreach ($attachedDocuments as $attachedDocument)
                                <tr class="">
                                    ...
                                    <td> {{ $paymentReq['related_document_request']['added_by']['first_name'] }}
                                        {{ $paymentReq['related_document_request']['added_by']['last_name'] }}</td>
                                </tr>
                            @endforeach
                        </tbody>
                    </table>
        ...
        @if (Session::has('userDetails') &&
                Session::get('userDetails')['id'] != $paymentReq['related_document_request']['added_by']['id']
        )
                    ...
                    @foreach ($paymentReq['approval_steps'] as $approver)
                        <div class="row mt-2">
                            <div class="col-6 fs-4">
                                @if ($approver['related_document_approval_step']['related_approval_step']['approval_step_name'] == 'REVIEW')
                                    <span class="fw-bold text-capitalize">Reviewed by:</span>
                                @elseif($approver['related_document_approval_step']['related_approval_step']['approval_step_name'] == 'VERIFICATION')
                                    <span class="fw-bold text-capitalize">Verified by:</span>
                                @elseif($approver['related_document_approval_step']['related_approval_step']['approval_step_name'] == 'APPROVAL')
                                    <span class="fw-bold text-capitalize">Approved by:</span>
                                @elseif($approver['related_document_approval_step']['related_approval_step']['approval_step_name'] == 'AUTHORISATION')
                                    <span class="fw-bold text-capitalize">Authorized by:</span>
                                @endif
                                {{ $approver['related_approver']['related_participant']['related_user']['first_name'] }}
                                {{ $approver['related_approver']['related_participant']['related_user']['last_name'] }}
                            </div>
                            <div class="col-6 fs-4">
                                <span
                                    class="fw-bold text-capitalize">{{ \Str::ucfirst(\Str::lower($approver['related_approver']['related_project_role']['role_name'])) }}</span>
                            </div>
                            <div class="col-12">
                                @if ($approver['status'])
                                    @if ($paymentReq['related_document_request']['status'] == 1)
                                        <div class="pt-2">
                                            <button type="submit"
                                                ...
                                                @if ($approver['related_document_approval_step']['related_approval_step']['approval_step_name'] == 'REVIEW')
                                                    <span class="fw-bold text-capitalize px-1">
                                                        Referred Back
                                                    </span>
                                                @elseif($approver['related_document_approval_step']['related_approval_step']['approval_step_name'] == 'VERIFICATION')
                                                    <span class="fw-bold text-capitalize px-3">
                                                        Referred Back
                                                    </span>
                                                @elseif($approver['related_document_approval_step']['related_approval_step']['approval_step_name'] == 'APPROVAL')
                                                    <span class="fw-bold text-capitalize px-1">
                                    ...
                                    @elseif ($paymentReq['related_document_request']['status'] == 2)
                                        <div class="pt-2">
                                            <button type="submit"
                                                class="btn btn-outline-primary text-capitalize px-2" disabled>
                                                <svg xmlns="http://www.w3.org/2000/svg" width="18" height="18"
                                                ...
                                                @if ($approver['related_document_approval_step']['related_approval_step']['approval_step_name'] == 'REVIEW')
                                                    <span class="fw-bold text-capitalize px-1">
                                                        {{ Str::lower($approver['related_document_approval_step']['related_approval_step']['approval_step_complete']) }}
                                                    </span>
                                                @elseif($approver['related_document_approval_step']['related_approval_step']['approval_step_name'] == 'VERIFICATION')
                                                    <span class="fw-bold text-capitalize px-3">
                                                        {{ Str::lower($approver['related_document_approval_step']['related_approval_step']['approval_step_complete']) }}
                                    ...
                                    @elseif($paymentReq['related_document_request']['status'] == 3)
                                        <div class="pt-2">
                                            <button type="submit"
                                ...
                                @else
                                    <div class="pt-2">
                                        <button type="button" id="blur"
                                            ...
                                            @if ($approver['related_document_approval_step']['related_approval_step']['approval_step_name'] == 'REVIEW')
                                                <span class="fw-bold text-capitalize px-2">
                                                    {{ Str::lower($approver['related_document_approval_step']['related_approval_step']['approval_step_action']) }}
                                                </span>
                                            @elseif($approver['related_document_approval_step']['related_approval_step']['approval_step_name'] == 'VERIFICATION')
                                                <span class="fw-bold text-capitalize px-3">
                                                    {{ Str::lower($approver['related_document_approval_step']['related_approval_step']['approval_step_action']) }}
