...
class IndexMemo extends Component
{
    public $projects = [];
    public $memoTypes = [];
    public $memoReqs = [];
    ...
    {
        return view('livewire.index-memo');
    }
    ...
    public function createMemoType($id)
    {
        Session::forget('memoId');
        Session::put('memoId', $id);
    ...
    public function reqByProject()
    {
        if ($this->project_id) {
            ...
            if ($this->tempClicked) {
                $jsonData = $this->getMemoTypes($this->project_id);
                if ($jsonData == 'Error') {
                    return redirect()->route('error');
                ...
                $jsonData = $this->getMemoReqs($this->project_id);
                if ($jsonData == 'Error') {
                    return redirect()->route('error');
                } elseif ($jsonData['status'] == 400) {
                    return redirect()
    ...
    public function handleRequest()
    {
        ...
        $projectId = Session::get('projectId');
        if ($projectId) {
            $jsonData = $this->getMemoReqs($projectId, $unAuthorized = 1);
            if ($jsonData) {
                if ($jsonData == 'Error') {
                    return redirect()->back()->with('error', 'Service Unavailable. Please try again later');
                } elseif ($jsonData['status'] == 400) {
    ...
    public function authorized()
    {
        ...
        $projectId = Session::get('projectId');
        if ($projectId) {
            $jsonData = $this->getMemoReqs($projectId, $authStatus = 2);
        } else {
            return redirect()->back()->with('warning', 'Select a project');
        }
        if ($jsonData) {
    ...
    public function unAuthorized()
    {
        ...
        $projectId = Session::get('projectId');
        if ($projectId) {
            $jsonData = $this->getMemoReqs($projectId, $unAuthStatus = 1);
        } else {
            return redirect()->back()->with('warning', 'Select a project');
        }
        if ($jsonData) {
    ...
    public function rejected()
    {
        ...
        $projectId = Session::get('projectId');
        if ($projectId) {
            $jsonData = $this->getMemoReqs($projectId, $rejected = 3);
        } else {
            return redirect()->back()->with('warning', 'Select a project');
        }
        if ($jsonData) {
    ...
    public function getMemoReqs($projectId, $statusFilter = 2)
    {
        $url = 'https://api.odms.savannah.ug/api/v1/projects/requests/memos?project_id=' . $projectId . '&user_filter=True&status_filter=' . $statusFilter;
        $accessToken = $this->getToken();
        if ($accessToken) {
            try {
                $response = Http::withHeaders([
