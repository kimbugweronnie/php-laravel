...
class CustomerService extends Controller
{
    ...
    public function store($request,$email,$phone_number,$phone_prefix,$password)
    {
        try {
            $user =  $this->user->createUser($request,$email,$phone_number,$phone_prefix,$password);
            $customer = $this->customer->createCustomer($request,$user->id);
    ...
    public function sendOtp($id)
    {
        ...
        $receiver = "$customer->phone_prefix$customer->phone_number";
        $message = "Your Loyalty points veritfication code is ".$otp."";
        $response = $client->sms()->send(
            new \Vonage\SMS\Message\SMS($receiver, 'LOYALTY', $message)  
        );
    ...
    public function authCheck($email,$pin_code)
    {
        ...
        if(!$user){
            return $this->messageSubscription("No user with username $username", 401);
        }
