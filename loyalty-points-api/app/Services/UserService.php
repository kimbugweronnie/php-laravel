...
class UserService extends Controller
{
    ...
    public function authcheck($email,$password)
    {
        ...
        //     return $this->send_message("Please verify email to continue", 401);
        // }
    ...
    public function createToken($email)
    {
        ...
        $accesstoken = auth()->user()->createToken('Anything',[ "*"],now()->addMinutes(240),$user->id,"App\\Models\\User");
        $user->access_token = $accesstoken->plainTextToken;
        $user->expires_in = 3600;
        // $user->expires_in = $accesstoken->accessToken->expires_at;
        $userdetails = [
            'id' => $user->id,
            'name' => $merchant ? $merchant->merchant_name : $customer->first_name.' '. $customer->last_name,
            'email' => $user->email,
            'user_type' => $merchant ? $merchant->user_type : $customer->user_type
        ];
        return $this->loginResponse($user->access_token, $userdetails, $user->expires_in, 200); 
    }
    ...
    public function getcustomers()
    {
        ...
        // return CustomerResource::collection($this->user->customers($role));  
    }
